import { DEBUG, error, log, toString, has } from "./utils";
...
export function validateDependency({ context }) {
  if (context.$router === void 0) {
    error(`Missing vue-router`, context);
  }
}

export function validateRoutePropsOption({ routeProps, context }) {
  let isValid = true;
...
export function validateRoutePropsDefaultOption({ routeProps, prop, context }) {
  if (
...
export function normalize({ routeProps }) {
...
export function generateData({ normalizedRouteProps, route, context }) {
  const data = {};
...
export function validateRoutePropsValue({ normalizedRouteProps, context }) {
  let isValid = true;
...
export function validateRequired({ normalizedRouteProps, prop, context }) {
...
export function validateType({ normalizedRouteProps, prop, context }) {
  const value = has(context.$route.query, prop)
    ? JSON.parse(context.$route.query[prop])
    : normalizedRouteProps[prop].default();
...
export function validateCustom({ normalizedRouteProps, prop, context }) {
  const value = has(context.$route.query, prop)
    ? JSON.parse(context.$route.query[prop])
    : normalizedRouteProps[prop].default();
